...
function doElection(stvDistrict) {
    //https://prfound.org/resources/reference/reference-meek-rule/
    

    //Start by getting the ballots
    ...
    const fnRound = function () {
        roundNum += 1;
        postMessage({progress: `Round ${roundNum}`});
        ...
        const fnIterate = function () {
            ...
            if (newWinners.length > 0) {
                postMessage({heading: `Round ${roundNum}-${iterationNum} - Elected: ${newWinners.map(w =>
    ...
    postMessage({heading: 'Final Winners', exhausted: totalExhausted, candidates: winners});
}
